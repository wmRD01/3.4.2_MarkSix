    ...
    execute: function () {
      ...
      _export("Rabbit", Rabbit = class Rabbit extends (_crd && StreamCipher === void 0 ? (_reportPossibleCrUseOfStreamCipher({
        ...
        _doReset() {
          ...
          const X = this._X = [K[0], K[3] << 16 | K[2] >>> 16, K[1], K[0] << 16 | K[3] >>> 16, K[2], K[1] << 16 | K[0] >>> 16, K[3], K[2] << 16 | K[1] >>> 16]; // Generate initial counter values

          const C = this._C = [K[2] << 16 | K[2] >>> 16, K[0] & 0xffff0000 | K[1] & 0x0000ffff, K[3] << 16 | K[3] >>> 16, K[1] & 0xffff0000 | K[2] & 0x0000ffff, K[0] << 16 | K[0] >>> 16, K[2] & 0xffff0000 | K[3] & 0x0000ffff, K[1] << 16 | K[1] >>> 16, K[3] & 0xffff0000 | K[0] & 0x0000ffff]; // Carry bit
          ...
          const IV = iv.words;
          const IV_0 = IV[0];
          const IV_1 = IV[1]; // Generate four sub vectors
        ...
        nextState() {
          ...
          const G = this.G; // Calculate new state values

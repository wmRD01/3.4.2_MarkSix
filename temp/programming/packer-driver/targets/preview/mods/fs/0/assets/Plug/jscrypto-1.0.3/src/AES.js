    ...
    execute: function () {
      ...
      _export("AES", AES = class AES extends (_crd && BlockCipher === void 0 ? (_reportPossibleCrUseOfBlockCipher({
        ...
        _doReset() {
          ...
          var keySize = key.nSigBytes / 4; // Compute number of rounds
        ...
        _doCryptBlock(words, offset, keySchedule, subMix0, subMix1, subMix2, subMix3, sBox) {
          ...
          var s3 = words[offset + 3] ^ keySchedule[3]; // Key schedule row counter

          var ksRow = 4; // Rounds

          for (var round = 1; round < nRounds; round++) {
            // Shift rows, sub bytes, mix columns, add round key
            var _s0 = subMix0[s0 >>> 24] ^ subMix1[s1 >>> 16 & 0xff] ^ subMix2[s2 >>> 8 & 0xff] ^ subMix3[s3 & 0xff] ^ keySchedule[ksRow++];

            var _s1 = subMix0[s1 >>> 24] ^ subMix1[s2 >>> 16 & 0xff] ^ subMix2[s3 >>> 8 & 0xff] ^ subMix3[s0 & 0xff] ^ keySchedule[ksRow++];

            var _s2 = subMix0[s2 >>> 24] ^ subMix1[s3 >>> 16 & 0xff] ^ subMix2[s0 >>> 8 & 0xff] ^ subMix3[s1 & 0xff] ^ keySchedule[ksRow++];

            var _s3 = subMix0[s3 >>> 24] ^ subMix1[s0 >>> 16 & 0xff] ^ subMix2[s1 >>> 8 & 0xff] ^ subMix3[s2 & 0xff] ^ keySchedule[ksRow++]; // Update state

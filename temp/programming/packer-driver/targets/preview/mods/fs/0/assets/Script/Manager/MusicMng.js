      ...
      MusicMng = class MusicMng extends (_crd && DataMusic === void 0 ? (_reportPossibleCrUseOfDataMusic({
        ...
        init() {
          ...
          return _asyncToGenerator(function* () {
            ...
            _this.gameString = (_crd && GameData === void 0 ? (_reportPossibleCrUseOfGameData({
              error: Error()
            ...
            _this.audio_Music = new Node().addComponent(AudioSource);
        ...
        sequenceEffectPlay(array) {
          ...
          return _asyncToGenerator(function* () {
            ...
            for (var index = 0; index < array.length; index++) {
              yield _this2.sequenceEffect(array[index]);
            }
          })();
        }

        sequenceEffect(effectName) {
          var _this3 = this;

          return _asyncToGenerator(function* () {
            return new Promise((resolve, reject) => {
              var getRandom = _this3.checkEffectRandom(effectName, random());

              var audio = _this3.getAudioSourcePool();

              audio.node.targetOff(audio.node);
              audio.clip = _this3.getClip(effectName);
              audio.volume = _this3.getEffectState() ? _this3._volEffect : 0;
              audio.play();
              var formatName = effectName + ":" + getRandom;
              audio.node.once(AudioSource.EventType.ENDED, () => {
                _this3.putAudioSourcePool(audio, formatName);
        ...
        effectPlay(effectName, playLoop) {
          if (playLoop === void 0) {
            playLoop = false;
          }
          ...
          audio.play();
          var formatName = effectName + ":" + getRandom; //資料儲存

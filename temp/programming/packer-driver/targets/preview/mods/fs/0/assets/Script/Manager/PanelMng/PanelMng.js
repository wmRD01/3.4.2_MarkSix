  ...
  return {
    ...
    execute: function () {
      ...
      _export("PanelMng", PanelMng = (_dec = ccclass('PanelMng'), _dec(_class2 = (_temp = class PanelMng {
              ...
              assetManager.loadBundle(bundleName, (err, bundle) => {
                bundle.loadDir("", Prefab, (err, data) => {
                  ...
                  data.forEach(_prefab => {
                    var _panel = instantiate(_prefab);

                    _this.parent.addChild(_panel);
                  });
                  bundle.release("");
                  resolve();
                });
        ...
        setPanel(str) {
          var _class = find(str, this.parent).getComponent(str);
        ...
        initSetting() {
          var len = this.tempBase.length; //排序

          for (var i = 0; i < len; i++) {
            for (var j = 0; j < len - i - 1; j++) {
              // 如果當前元素大於下一個元素，則交換它們
              if (this.tempBase[j].zIndex > this.tempBase[j + 1].zIndex) {
                var temp = this.tempBase[j];
          ...
          for (var index = 0; index < this.tempBase.length; index++) {

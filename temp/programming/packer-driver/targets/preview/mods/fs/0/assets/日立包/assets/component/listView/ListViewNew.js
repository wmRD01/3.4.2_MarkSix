        ...
        */
        ...
        initialize() {
          ...
          if (this.direction == this.DirectionVertical) {
            ...
            for (var i = 0; i < this.spawnCount; ++i) {
              ...
              var x = (this.itemWidth + this.spacingX) * (-(this.colNum - 1) * 0.5 + col);
            ...
            for (var _i = 0; _i < this.spawnCount; ++_i) {
              // spawn items, we only need to do this once
              var _item = instantiate(this.itemPrefab);

              this.content.addChild(_item);
              _item.active = true; // 设置该item的坐标（注意父节点content的Anchor坐标是(0, 0.5)，所以item的x坐标总是正值）

              var _col = Math.floor(_i / this.rowNum);

              var _row = _i % this.rowNum;

              var y = (this.itemHeight + this.spacingY) * ((this.rowNum - 1) * 0.5 - _row);

              _item.setPosition(this.itemWidth * (0.5 + _col) + this.spacingX * (_col + 1), y);

              _item.getComponent(_crd && ListViewItemNew === void 0 ? (_reportPossibleCrUseOfListViewItemNew({
                error: Error()
              }), ListViewItemNew) : ListViewItemNew).index = _i;

              _item.getComponent(_crd && ListViewItemNew === void 0 ? (_reportPossibleCrUseOfListViewItemNew({
                error: Error()
              }), ListViewItemNew) : ListViewItemNew).updateItem(_i, this.dataList[_i], this.obj);
        ...
        update(dt) {
          ...
          if (this.direction == this.DirectionVertical) {
            ...
            var isDown = this.content.position.y < this.lastContentPosY; // 实际创建项占了多高（即它们的高度累加）

            var offset = (this.itemHeight + this.spacingY) * Math.ceil(this.spawnCount / this.colNum);
            var newY = 0; // 遍历数组，更新item的位置和显示
            ...
            for (var i = 0; i < items.length; ++i) {
              ...
              if (isDown) {
                ...
                if (viewPos.y < -this.bufferZoneY && newY < 0) {
                  items[i].y = newY;
                  var itemComp = items[i].getComponent(_crd && ListViewItemNew === void 0 ? (_reportPossibleCrUseOfListViewItemNew({
                    error: Error()
                  }), ListViewItemNew) : ListViewItemNew);
                  var index = itemComp.index - items.length; // 更新index及数据下标
                ...
                if (viewPos.y > this.bufferZoneY && newY > -this.contentUI.height) {
                  ...
                  var _itemComp = items[i].getComponent(_crd && ListViewItemNew === void 0 ? (_reportPossibleCrUseOfListViewItemNew({
                    error: Error()
                  }), ListViewItemNew) : ListViewItemNew);

                  var _index = _itemComp.index + items.length; // 更新index及数据下标


                  _itemComp.index = _index;

                  _itemComp.updateItem(_index, this.dataList[_index], this.obj);
            ...
            var isLeft = this.content.position.x < this.lastContentPosX; // 实际创建项占了多高（即它们的宽度累加）

            var _offset = (this.itemWidth + this.spacingX) * Math.ceil(this.spawnCount / this.rowNum);

            var newX = 0; // 遍历数组，更新item的位置和显示

            for (var _i2 = 0; _i2 < items.length; ++_i2) {
              var _viewPos = this.getPositionInView(items[_i2]);

              if (isLeft) {
                // 提前计算出该item的新的y坐标
                newX = items[_i2].x + _offset; // 如果往左滚动时item已经超出缓冲矩形，且newX未超出content右边界，
                // 则更新item的坐标（即右移了一个offset的位置），同时更新item的显示内容

                if (_viewPos.x < -this.bufferZoneX && newX < this.contentUI.width) {
                  items[_i2].x = newX;

                  var _itemComp2 = items[_i2].getComponent(_crd && ListViewItemNew === void 0 ? (_reportPossibleCrUseOfListViewItemNew({
                    error: Error()
                  }), ListViewItemNew) : ListViewItemNew);

                  var _index2 = _itemComp2.index + items.length; // 更新index及数据下标


                  _itemComp2.index = _index2;

                  _itemComp2.updateItem(_index2, this.dataList[_index2], this.obj);
                }
              } else {
                // 提前计算出该item的新的y坐标
                newX = items[_i2].x - _offset; // 如果往右滚动时item已经超出缓冲矩形，且newY未超出content左边界，
                // 则更新item的坐标（即左移了一个offset的位置），同时更新item的显示内容

                if (_viewPos.x > this.bufferZoneX && newX > 0) {
                  items[_i2].x = newX;

                  var _itemComp3 = items[_i2].getComponent(_crd && ListViewItemNew === void 0 ? (_reportPossibleCrUseOfListViewItemNew({
                    error: Error()
                  }), ListViewItemNew) : ListViewItemNew);

                  var _index3 = _itemComp3.index - items.length; // 更新index及数据下标


                  _itemComp3.index = _index3;

                  _itemComp3.updateItem(_index3, this.dataList[_index3], this.obj);
                }
              }
            } // 更新lastContentPosY和总项数显示
        ...
        updateData(list, isSplitLoad, notScrollToTop) {
          this.dataList = list;
          var NotScrollToTop = this.dataList && this.dataList.length > this.spawnCount;
          ...
          if (this.direction == this.DirectionVertical) {
            ...
            for (var i = 0; i < this.itemList.length; i++) {
              ...
              var itemComp = this.itemList[i].getComponent(_crd && ListViewItemNew === void 0 ? (_reportPossibleCrUseOfListViewItemNew({
                error: Error()
              }), ListViewItemNew) : ListViewItemNew);
            ...
            for (var _i3 = 0; _i3 < this.itemList.length; _i3++) {
              var col = Math.floor(_i3 / this.rowNum);
              this.itemList[_i3].x = this.itemWidth * (0.5 + col) + this.spacingX * (col + 1);

              var _itemComp4 = this.itemList[_i3].getComponent(_crd && ListViewItemNew === void 0 ? (_reportPossibleCrUseOfListViewItemNew({
                error: Error()
              }), ListViewItemNew) : ListViewItemNew);

              var _data = this.dataList[_i3];
              _itemComp4.index = _i3;

              _itemComp4.updateItem(_i3, _data, this.obj);
            }
        ...
        updateDataNoChangeIndex(list) {
          ...
          for (var i = 0; i < this.itemList.length; i++) {
            ...
            var data = this.dataList[itemComp.index];
        ...
        updateDataAndSplitLoad(list) {
          ...
          if (this.direction == this.DirectionVertical) {
            ...
            this.callBack = () => {
              if (_i4 >= this.itemList.length || !isValid(this.content)) {
                this.unschedule(this.callBack);
                this.callBack = null;
                this.initialled = true;
              ...
              var col = Math.floor(_i4 / this.rowNum);
              this.itemList[_i4].x = this.itemWidth * (0.5 + col) + this.spacingX * (col + 1);

              var itemComp = this.itemList[_i4].getComponent(_crd && ListViewItemNew === void 0 ? (_reportPossibleCrUseOfListViewItemNew({
                error: Error()
              }), ListViewItemNew) : ListViewItemNew);

              itemComp.index = _i4;
              var data = this.dataList[_i4];
              itemComp.updateItem(_i4, data, this.obj);
              _i4++;
            };
          }
        ...
        updateHeightForData(data) {
          ...
          if (this.dataList) {
            ...
            if (this.direction == this.DirectionVertical) {
              ...
              var viewHeight = this.content.parent.getComponent(UITransform).height;
              ...
              var viewWidth = this.content.parent.getComponent(UITransform).width;
